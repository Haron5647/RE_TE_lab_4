...
# 3.3. Бинарные категориальные колонки (Yes/No), кроме целевой
binary_cols = ['Participation in Extracurricular Activities']
for col in binary_cols:
    df[col] = df[col].map({'Yes': 1, 'No': 0})
    df[col] = df[col].fillna(-1)  # для бинарных признаков можно оставить -1 как отдельный класс

# 3.4. Целевая переменная Passed
target_col = 'Passed'
df[target_col] = df[target_col].map({'Yes': 1, 'No': 0})
# удаляем строки, где Passed пропущен
df = df[df[target_col].notna()]

# 3.5. Остальные категориальные колонки
categorical_cols = df.select_dtypes(include=['object']).columns.tolist()
categorical_cols = [col for col in categorical_cols if col not in binary_cols + ['Student ID']]
df[categorical_cols] = df[categorical_cols].fillna('Unknown')

# ------------------ 3.6. Label Encoding для категориальных признаков ------------------
le = LabelEncoder()
...
# ------------------ 9. Проверка мультиколлинеарности ------------------
