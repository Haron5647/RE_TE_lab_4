...
OUT_DIR = "clustering_results_BayesSearchCV_UMAP"
RANDOM_STATE = 42
...
# UMAP для снижения размерности до 2–10 признаков (можно выбрать n_components=2 для визуализации)
umap_model = umap.UMAP(
    n_neighbors=30,
    min_dist=0.1,
    n_components=5,  # можно менять, например 2–5
    metric='euclidean',
...
labels_birch_best = birch_search.best_estimator_.predict(X_umap)
...
labels_dbscan_best = dbscan_search.best_estimator_.predict(X_umap)

# ------------------ Сохраняем все метки кластеров ------------------
out_labels = pd.DataFrame({
    'Birch_base': Birch(n_clusters=None, threshold=0.5, branching_factor=50).fit(X_umap).predict(X_umap),
    'DBSCAN_base': DBSCAN(eps=0.5, min_samples=5).fit_predict(X_umap),
    'Birch_bayes_best': labels_birch_best,
    'DBSCAN_bayes_best': labels_dbscan_best,
    'Passed': y
})
out_labels.to_csv(os.path.join(OUT_DIR, "clusters_labels_bayes_umap.csv"), index=False)

print("\n✔ Все модели обучены и сохранены. Результаты кластеров сохранены в clusters_labels_bayes_umap.csv")
